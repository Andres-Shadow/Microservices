Feature: la API provee al usuario la funcionalidad de ingresar con sus credenciales previamente registradas
    ...
    Scenario: pepe quiere autenticarse dentro de la aplicación 
        When invoca el método de autenticación en /api/v1/login 
        And los datos diligenciados no existen en la base de datos
        Then se obtiene el mensaje de respuesta 401
        And se obtiene el mensaje de error "Usuario no encontrado"
    
    Scenario: pepe quiere autenticarse dentro de la aplicación 
        When invoca el método de autenticación en /api/v1/login 
        And la contraseña ingresada no coincide con los registrados en la base de datos
        Then se obtiene el mensaje de respuesta 401
        And se obtiene el mensaje de error "Contraseña incorrecta"

    Scenario: pepe quiere autenticarse dentro de la aplicación 
        When invoca el método de autenticación en /api/v1/login
        And los datos diligenciados no cumplen con el formato esperado
